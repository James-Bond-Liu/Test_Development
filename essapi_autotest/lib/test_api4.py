...
class TestApi(unittest.TestCase):
    ...
    def test_api(self, item):
        ...
        if item['sql_before'] is not None:
            ...
            result1 = \
            DoMysql(eval(ReadConfig().readconfig('MYSQL_DB', item['sql_before'][0]))).select(item['sql_before'][2])[0]
            setattr(GlobalData, item['sql_before'][1], result1)
            logger.debug(
                f'测试用例{item["case_name"]}的sql_before不为空，储存变量{item["sql_before"][1]}={getattr(GlobalData, item["sql_before"][1])}')
        ...
        try:
            ...
            logger.info('测试用例{}响应状态码status_code为200'.format(item['case_name']))
            self.assertEqual(res.json()['msg_code'], item['request_expect_result'])
            logger.info('测试用例{}响应报文operate.success'.format(item['case_name']))
            if item['sql_after'] is not None:
                item['sql_after'] = DoRegx().do_regx(item['sql_after'])
                item['sql_after'] = eval(item['sql_after'])
                logger.debug('****sql'.format(item['sql_after'][2]))
                result2 = \
                DoMysql(eval(ReadConfig().readconfig('MYSQL_DB', item['sql_after'][0]))).select(item['sql_after'][2])[0]
                setattr(GlobalData, item['sql_after'][1], result2)
                logger.debug(
                    f'测试用例{item["case_name"]}的sql_after不为空，储存变量{item["sql_after"][1]}={getattr(GlobalData, item["sql_after"][1])}')
            ...
            if item['database_compare_sql'] is not None:
                ...
                logger.debug('测试用例{}查询数据库结果为{}，期望结果为{}'.format(item['case_name'], database_actual_result, item['database_expect_result']))
                if database_actual_result == item['database_expect_result']:
                    compare_result = 'Pass'
                    logger.info(f'测试用例{item["case_name"]}断言结果为{compare_result}')
                else:
                    compare_result = 'Failed'
                    logger.error(f'测试用例{item["case_name"]}断言结果为{compare_result}')
            else:
                compare_result ='Pass'
                logger.info(f'测试用例{item["case_name"]}断言结果为{compare_result}')
            ...
            compare_result = 'Failed'
            logger.error('测试用例{}断言过程报错{}'.format(item['case_name'], e))
            raise e
        finally:
            try:
