...
class TestApi(unittest.TestCase):
    ...
    def test_api(self, item):
        ...
        if item['sql_before']:
            sql_result1 = DoMysql(eval(read_conf.readconfig('MYSQL_DB', item['database1']))).select(item['sql_before'])[0]
            if item['path_info'].find('${customer_no}') != -1:
                item['path_info'] = item['path_info'].replace('${customer_no}', str(sql_result1))
            elif item['parameters'] is not None and item['parameters'].find('${code}') != -1:
                item['parameters'] = item['parameters'].replace('${code}', str(sql_result1))
            elif item['path_info'].find('${organization_id}') != -1:
                item['path_info'] = item['path_info'].replace('${organization_id}', str(sql_result1))
            elif item['parameters'] is not None and item['parameters'].find('${organization_id}') != -1:
                item['parameters'] = item['parameters'].replace('${organization_id}', str(sql_result1))
